...
func transformBereqURL(tx *varnishTransaction, rec varnishlog.Record) error {
	return transformReqURL(tx, rec)
}
...
func transformBereqMethod(tx *varnishTransaction, rec varnishlog.Record) error {
	return transformReqURL(tx, rec)
}
...
func transformBerespReason(tx *varnishTransaction, rec varnishlog.Record) error {
	return transformRespReason(tx, rec)
}
...
func transformBerespStatus(tx *varnishTransaction, rec varnishlog.Record) error {
	return transformRespStatus(tx, rec)
}
...
func transformBereqProtocol(tx *varnishTransaction, rec varnishlog.Record) error {
	return transformReqProtocol(tx, rec)
}
...
func transformBackendOpen(tx *varnishTransaction, rec varnishlog.Record) error {
	...
	beConnStatus := trimUnprintableChars(parts[6])
	beConnReused := false
	if beConnStatus == "reused" {
		beConnReused = true
	}

	tx.Backend.Addr = trimUnprintableChars(beAddr)
	tx.Backend.Port = bePort
	tx.Backend.ConnReused = beConnReused
	tx.Backend.Name = trimUnprintableChars(beName)
	return nil
}
...
func transformHit(tx *varnishTransaction, rec varnishlog.Record) error {
	...
	if scanned, err := fmt.Sscanf(trimUnprintableChars(rec.Payload.String()), "%d %f %f %f %d %d",
		&xid, &ttl, &grace, &keep, &fetched, &clen); err != nil || scanned < 4 {
...
var (
	transformFuncs = map[string]varnishTagTransformerFunc{
		...
		"BereqURL":      transformBereqURL,
		"ReqMethod":     transformReqMethod,
		"BereqMethod":   transformBereqMethod,
		"Link":          transformLink,
		"RespReason":    transformRespReason,
		"BerespReason":  transformBerespReason,
		"RespStatus":    transformRespStatus,
		"BerespStatus":  transformBerespStatus,
		...
		"BereqProtocol": transformBereqProtocol,
...
func updateSpan(span ptrace.Span, tx *varnishTransaction) error {
	if tp, ok := tx.Req.Headers["traceparent"]; ok {
		if sid, tid, err := extractTraceparent(tp); err != nil {
			return err
		} else {
			span.SetTraceID(sid)
			span.SetSpanID(tid)
